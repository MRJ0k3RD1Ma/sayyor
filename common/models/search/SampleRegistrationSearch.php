...
class SampleRegistrationSearch extends SampleRegistration
{
    ...
    {
        return [
            [['id', 'is_research', 'code_id', 'research_category_id','status_id', 'results_conformity_id', 'organization_id', 'emp_id', 'reg_id'], 'integer'],
    ...
    {
        $query = SampleRegistration::find()->select(['sample_registration.*','route_sert.status_id as route_status'])
            ->innerJoin('route_sert','route_sert.registration_id=sample_registration.id and route_sert.leader_id='.Yii::$app->user->id)
            ->groupBy('sample_registration.id')
        ...
        if($this->status_id != -1){
            $query->andWhere(['route_sert.status_id'=>$this->status_id]);
        }
        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
    ...
    {
        $query = SampleRegistration::find()->select(['sample_registration.*','route_sert.status_id as route_status'])
            ->innerJoin('route_sert','route_sert.registration_id=sample_registration.id and route_sert.executor_id='.Yii::$app->user->id)
            ->groupBy('sample_registration.id')
            ->orderBy(['created'=>SORT_DESC]);
    ...
    {
        $query = SampleRegistration::find()->select(['sample_registration.*','route_sert.status_id as route_status'])
            ->innerJoin('route_sert','route_sert.registration_id=sample_registration.id and route_sert.director_id='.Yii::$app->user->id)
            ->groupBy('sample_registration.id')
            ->orderBy(['created'=>SORT_DESC]);
