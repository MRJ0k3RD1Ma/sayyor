...
//            'director_id',
            [
                'attribute' => 'code',
                'value' => function ($d) {
                    $url = Yii::$app->urlManager->createUrl(['/director/viewreganimal', 'id' => $d->id]);
            ...
            [
                ...
                'value'=>function($d){
                    $res = "";
                    foreach (\common\models\RouteSert::find()->where(['registration_id'=>$d->id])->andWhere(['director_id'=>Yii::$app->user->id])->all() as $item){
                        $res .= "{$item->status->class} {$item->sample->kod}<br>";
                    }
                    return $res;
                },
                'format'=>'raw'
