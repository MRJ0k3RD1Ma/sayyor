...
//            'director_id',
            [
                'attribute' => 'code',
                'value' => function ($d) {
                    $url = Yii::$app->urlManager->createUrl(['/lab/viewregfood', 'id' => $d->id]);
            ...
            [
                ...
                'value'=>function($d){
                    $res = "";
                    foreach (\common\models\FoodRoute::find()->where(['registration_id'=>$d->id])->andWhere(['executor_id'=>Yii::$app->user->id])->all() as $item){
