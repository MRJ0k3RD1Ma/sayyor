...
//            'director_id',
            [
                'attribute' => 'code',
                'value' => function ($d) {
                    $url = Yii::$app->urlManager->createUrl(['/leader/viewreganimal', 'id' => $d->id]);
            ...
            [
                ...
                'value'=>function($d){
                    $res = "";
                    foreach (\common\models\RouteSert::find()->where(['registration_id'=>$d->id])->andWhere(['leader_id'=>Yii::$app->user->id])->all() as $item){
