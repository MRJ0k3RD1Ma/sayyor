...
?>
    <div class="sertificates-update">
        ...
        <h4><?php foreach ($samples as $item){echo $item->kod.' '; }?> <?= Yii::t('register', 'Raqamli namunalar bo`yicha topshiriqlar ro`yhati') ?>
            <span style="float:right"><a href="<?= Yii::$app->urlManager->createUrl(['/register/incomeanimalmulti','id'=>$reg->id])?>" class="btn btn-primary">Yana topshiriq qo'shish</a></span>
        </h4>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                <?php $n=0; foreach (\common\models\RouteSert::find()->where(['registration_id'=>$reg->id,'is_group'=>1])->orderBy('leader_id')->all() as $item): $n++?>
                    <tr>
                        <td><?= $n?></td>
                        <td><?= $item->leader->name ?></td>
                        <td><?= $item->status->name_uz?></td>
                        <td>
                            <a href="<?= Yii::$app->urlManager->createUrl(['/register/incomeanimalmulti','id'=>$reg->id,'leader_id'=>$item->leader_id])?>" class="btn btn-primary"><span class="fa fa-edit"></span></a>
                        </td>
                    </tr>
                <?php endforeach;?>
                </tbody>
        ...
        <?php if($leader_id){
            ...
            echo $this->render('incomemulti/_change',[
                'model'=>$model,
                'route'=>$route,
                'cs'=>$cs,
                'result'=>$result,
            ...
            echo $this->render('incomemulti/_new',[
                'model'=>$model,
                'route'=>$route,
                'cs'=>$cs,
                'result'=>$result,
