...
<?php foreach ($samples as $item){echo $item->kod.' '; }?> <?= Yii::t('register', 'Raqamli namunalarni topshiriq ma`lumotlarini o`zgartirish') ?>
<?php
$lg = 'uz';
...
<?php }?>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-hover table-bordered">
            ...
            <tbody>
            <?php foreach (\common\models\ResultAnimalTests::find()->where('route_id in (select id from route_sert where leader_id='.$leader_id.' and registration_id='.$reg->id.' and is_group=1)')->all() as $item): ?>
                <tr>
