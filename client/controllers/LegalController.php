...
class LegalController extends Controller
{
    ...
    public function actionDeleteanimal($id){
        $sid = 0;
        if($model = Samples::findOne($id)){
            $sid = $model->sert_id;
            if($model->status_id == 0){
                $model->delete();
                Yii::$app->session->setFlash('success','Namuna muvoffaqiyatli o`chirildi');
            }else{
                Yii::$app->session->setFlash('error','Namuna muvoffaqiyatli o`chirildi');
            }
        }
        if($sid == 0){
            return $this->redirect(['/legal/listanimal']);
        }else{
            return $this->redirect(['/legal/view','id'=>$sid]);
        }
    }

    public function actionDeletefood($id){
        $sid = 0;
        if($model = FoodSamples::findOne($id)){
            $sid = $model->sert_id;
            if($model->status_id == 0){
                $model->delete();
                Yii::$app->session->setFlash('success','Namuna muvoffaqiyatli o`chirildi');
        ...
        if($sid == 0){
            return $this->redirect(['/legal/listfood']);
        }else{
            return $this->redirect(['/legal/viewfood','id'=>$sid]);
        }
    }
