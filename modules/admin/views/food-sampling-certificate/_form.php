...
<div class="food-sampling-certificate-form">
    ...
    <?= $form->field($model, 'organization_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\Organizations::find()->all(),'id','NAME_FULL'),['prompt'=>'Tashkilotni tanlang']) ?>
    ...
    <?php if($model->sampling_site){
        ...
        <?= $form->field($model, 'district')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\DistrictView::find()->where(['region_id'=>$model->samplingSite->soato0->region_id])->all(),'district_id','name_'.$ads),['prompt'=>Yii::t('cp.vetsites','Tumanni tanlang')]) ?>
        <?= $form->field($model, 'soato')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\QfiView::find()->where(['district_id'=>$model->samplingSite->soato0->district_id])->all(),'MHOBT_cod','name_'.$ads),['prompt'=>Yii::t('cp.vetsites','QFYni tanlang')]) ?>
        <?= $form->field($model, 'sampling_site')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\VetSites::find()->where(['soato'=>$model->samplingSite->soato0->qfi_id])->all(),'id','name'),['prompt'=>Yii::t('cp.vetsites','Vet uchstkani tanlang')]) ?>
    ...
    <?= $form->field($model, 'unit_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\Units::find()->all(),'id','name_uz'),['prompt'=>'Birlikni tanlang']) ?>
    ...
    <?= $form->field($model, 'verification_pupose_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\VerificationPurposes::find()->all(),'id','name_uz'),['prompt'=>'Tekshirishdan maqsadni tanlang']) ?>

    <?= $form->field($model, 'sample_box_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\SampleBoxes::find()->all(),'id','name_uz'),['prompt'=>'Namuna o\'ramini tanlang']) ?>

    <?= $form->field($model, 'sample_condition_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\SampleConditions::find()->all(),'id','name_uz'),['prompt'=>'Namuna holati']) ?>
    ...
    <?= $form->field($model, 'laboratory_test_type_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\LaboratoryTestType::find()->all(),'id','name_uz'),['prompt'=>'Laboratoriya tadqiqot turini tanlang']) ?>
